(module dotfiles.scratch.plugins
  {require {a aniseed.core}})

(defn- astronauta-config []
  (require :astronauta.keymap))
(defn- stabilize-config []
...
(def- plugin-map
  {:wbthomason/packer.nvim {}
   ...
   :luukvbaal/stabilize.nvim {:config stabilize-config}
...
; list values are either :user/repo string or
; {config} table for the previous string... so:
; add {string {empty}} to `tbl` when `v` is a
; string, if `v` is a table then override the
; previous {empty} with `v`
...
; this is used in dotfiles.plugins now
; [k] => {1 k} so merging [k] into the
; config returns {1 :user/repo ...config}
(defn- plugin-map-to-spec [plugins]
  (->> plugins (a.map-indexed
                 (fn [[k v]]
                   (a.merge [k] v)))))

(assert (vim.deep_equal plugin-map (plugin-list-to-map plugin-list)))
(assert (vim.deep_equal
          (-> plugin-map (plugin-map-to-spec))
